...
Lines indented deeper than the `key |` opener are now treated as block
text. Previously, TextMate fell back to `level1-keys` / `list-item`
rules and highlighted the first word of every indented line ("Reasons",
"Default", "Confidence" …) as a separate property. The new
`multiline-block` pattern uses a `\1`-backreferenced `while` clause so
the block ends exactly when the indent drops to (or below) the opener
level — matching the runtime parser semantics in
`synx_core::parser::parse`.
