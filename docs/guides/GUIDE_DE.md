...
## C++ SDK (`bindings/cpp`) — v3.6.0

Der C++-Teil ist eine **C++17**-Headerdatei `include/synx/synx.hpp` über die nativ mit **`synx-c`** (Rust `synx-core`) gebundene Bibliothek — gleiche Grammatik, `!active`, `!tool`, `.synxb`, kanonisches JSON, `diff`.

```bash
cargo build -p synx-c --release
```

Include-Pfade: `bindings/cpp/include`, `bindings/c-header/include`. Details: [`bindings/cpp/README.md`](../../bindings/cpp/README.md).

## Go (`bindings/go`, cgo) — v3.6.0

Go-Binding über **cgo** und **`synx-c`** — gleicher Motor wie Rust. `CGO_ENABLED=1`, `cargo build -p synx-c --release`. Unter **Windows** `CGO_LDFLAGS`/`PATH` zur DLL siehe [`bindings/go/README.md`](../../bindings/go/README.md).

## Swift (`bindings/swift`, SwiftPM) — v3.6.0

SwiftPM-Paket **`Synx`** mit **C-Interop (`CSynx`)** zu **`synx-c`** — gleicher Motor wie Rust, kein reiner Swift-Parser. `cargo build -p synx-c --release`, dann `swift build` / `swift test` mit `-Xlinker -L … -lsynx_c`; macOS: ggf. `DYLD_LIBRARY_PATH`. [`bindings/swift/README.md`](../../bindings/swift/README.md).

## Kotlin / JVM (`bindings/kotlin`, JNA) — v3.6.0

**`SynxEngine`** lädt **`synx-c`** per **JNA** — gleicher Motor wie Rust. `SYNX_LIB_DIR` → `target/release`, `./gradlew test`, **JDK 17+**. [`bindings/kotlin/README.md`](../../bindings/kotlin/README.md).

## Mojo (`bindings/mojo`) — v3.6.0

**Mojo → CPython `synx_native`** (wie PyO3-Python hier im Repo): gleicher Motor wie `synx-core`, keine zweite Grammatik in purem Mojo. `pip install synx-format`; siehe [`bindings/mojo/README.md`](../../bindings/mojo/README.md).
