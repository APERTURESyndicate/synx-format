...
> **🔗 [View logo →](https://media.aperturesyndicate.com/asother/as/branding/png/aperturesyndicate.png)**

<h1 align="center">SYNX v3.6 — The Complete Guide</h1>
...
- [Binary Format (`.synxb`)](#-binary-format-synxb)
...
- [C# / .NET Parser (`Synx.Core`)](#-c--net-parser-synxcore)
...
Download the `.vsix` from [GitHub Releases](https://github.com/APERTURESyndicate/synx-format/releases) and double-click to install.
...
SYNX v3.6 ships with **24 markers**. Each marker is a function attached to a key with `:marker` syntax.
...
SYNX supports a compact binary representation called `.synxb`. It preserves all data (values, types, metadata, includes, constraints) with **full round-trip fidelity** and achieves **40%+ size reduction** vs text.

### Wire Format

```
Offset  Size   Description
0       5      Magic bytes: "SYNXB"
5       1      Format version (currently 1)
6       1      Flags: bit 0=active, bit 1=locked, bit 2=has_metadata, bit 3=resolved
7       4      Uncompressed payload size (u32 LE)
11      N      Deflate-compressed payload
```

Payload contains:
1. **String interning table** — every unique string stored once, referenced by varint index
2. **Value tree** — type-tagged values with zigzag-encoded integers, IEEE 754 floats
3. **Metadata + includes** (if `has_metadata` flag is set)
...
// Compile text → binary
let binary: Vec<u8> = Synx::compile("!active\nserver\n  port 8080\n", false);

// Check magic
assert!(Synx::is_synxb(&binary));

// Decompile binary → text
let text: String = Synx::decompile(&binary).unwrap();
```

**Node.js:**

```js
const { compile, decompile, isSynxb } = require('@aperturesyndicate/synx-format-node');

const buf = compile('server\n  port 8080\n');
console.log(isSynxb(buf));         // true
console.log(decompile(buf));       // "server\n  port 8080\n"
```
...
data = synx_native.compile("server\n  port 8080\n")
assert synx_native.is_synxb(data)
text = synx_native.decompile(data)
```

### Resolved Mode

Pass `resolved=true` to strip metadata/includes and store only resolved values:

```rust
let snapshot = Synx::compile(active_config, true);
// No :env, :ref, :calc markers — just final values
```

This is useful for deploying a "frozen" config that doesn't depend on runtime resolution.
...
The `!tool` directive tells the parser that the file (or block) describes an **LLM tool call**. The first key is the tool name, and the remaining keys are parameters. This is drastically more compact than JSON for tool use — saving tokens, reducing latency, and making tool calls human-readable.
...
**Parsed output** (via `Synx::parse_tool()` or binding `parse_tool()`):
...
The equivalent JSON tool call is **61% larger**:
...
```json
{
  ...
  "function": {
    "name": "web_search",
    "arguments": "{\"query\": \"latest Rust release\", \"lang\": \"en\", \"max_results\": 5}"
  }
}
```
...
Define available tools for an LLM agent:
...
{
  "tools": [
    { "name": "memory_write", "params": { "path": "string", "value": "string" } },
    { "name": "web_search", "params": { "query": "string", "lang": "string", "max_results": "int" } }
  ]
}
```

### `!tool` + `!active` — Dynamic Parameters

Combine `!tool` with `!active` to use markers in tool parameters:
...
Markers like `:env`, `:default`, `:calc` are resolved **before** the tool output is reshaped.
...
let result = Synx::parse_tool("!tool\nweb_search\n  query test\n", &Options::default());
// result["tool"] == "web_search"
// result["params"]["query"] == "test"
```

**Node.js:**

```js
const { parseTool } = require('@aperturesyndicate/synx-format-node');
...
result = synx_native.parse_tool("!tool\nweb_search\n  query test\n")
# result["tool"] == "web_search"
# result["params"]["query"] == "test"
```
...
```c
char* json = synx_parse_tool("!tool\nweb_search\n  query test\n");
// json: {"params":{"query":"test"},"tool":"web_search"}
synx_free(json);
```

**C++** (`bindings/cpp/include/synx/synx.hpp`) — same engine via `synx::parse_tool`:

```cpp
#include <synx/synx.hpp>
auto j = synx::parse_tool("!tool\nweb_search\n  query test\n");
// *j is JSON with tool + params
```

**Go** (`bindings/go`, cgo) — same engine:
...
**Swift** (`bindings/swift`, SwiftPM) — same engine via `SynxEngine`:
...
**Kotlin/JVM** (`bindings/kotlin`, JNA) — same engine via `SynxEngine`:
...
**Mojo** (`bindings/mojo`) — same engine via CPython `synx_native`:

```mojo
from synx.interop import parse_tool_json
# parse_tool_json("!tool\nweb_search\n  query test\n")
```

### Binary Format Support

`!tool` and `!schema` flags are preserved in `.synxb` binary files. Compiling a `!tool` file and decompiling it restores the directives.

---

## 🧠 C++ SDK (`bindings/cpp`)

> v3.6.0 — parity with **`synx-core`** (not a second parser).

The C++ layer is a **C++17 header** (`include/synx/synx.hpp`) over the **`synx-c`** C library built from **`bindings/c-header`**. You get the same **`!active`** resolution, **`!tool`** reshape, **`.synxb`**, **canonical JSON**, and **structural `diff`** as Rust — because the implementation **is** `synx-core`.

**1. Build the native library** (repo root):

```bash
cargo build -p synx-c --release
```

**2. Include paths:** `bindings/cpp/include` and `bindings/c-header/include`.

**3. Link:** the produced `synx_c` shared or static library for your platform (`libsynx_c.so` / `libsynx_c.dylib` / `synx_c.dll`).
...
int main() {
    auto json = synx::parse_active("!active\nport:env:default:8080 PORT\n");
    if (!json) return 1;
    std::cout << *json << '\n';
    return 0;
}
...
**API:** `synx::parse`, `parse_active`, `stringify`, `format`, `parse_tool`, `diff`, `compile`, `decompile`, `is_synxb` — see `bindings/cpp/README.md` and `bindings/cpp/CMakeLists.txt` (optional `synx_cpp_minimal` example; set `SYNX_C_LIBRARY` when configuring CMake).

---

## 🐹 Go (`bindings/go`, cgo)

> v3.6.0 — **full parity** with **`synx-core`** via **`synx-c`** (not a pure-Go rewrite).

Enable cgo (`CGO_ENABLED=1`), build the native library from the repo root (`cargo build -p synx-c --release`), then import:

```go
import synx "github.com/APERTURESyndicate/synx-format/bindings/go"
```

**API:** `synx.Parse`, `ParseActive`, `Stringify`, `Format`, `ParseTool`, `Compile`, `Decompile`, `IsSynxb`, `Diff` — all return `(string, error)` or `([]byte, error)`; `error` is `synx.ErrSynx` when the C layer returns NULL.

**Linux/macOS:** default link flags point at `target/release` next to the repo (`-lsynx_c`).

**Windows (Rust MSVC + MinGW gcc):** set `CGO_LDFLAGS` to your `synx_c.dll.lib`, `CGO_LDFLAGS_ALLOW=.*`, and put `target\release` on `PATH` so `synx_c.dll` loads — see [`bindings/go/README.md`](../../bindings/go/README.md).
...
> v3.6.0 — **full parity** with **`synx-core`** via **`synx-c`** (SwiftPM target **`Synx`**, C module **`CSynx`** — not a pure-Swift grammar port).

**1. Build the native library** (repo root): `cargo build -p synx-c --release`.

**2. Build / test** from `bindings/swift` with linker flags pointing at `target/release` (`-L … -lsynx_c`). On **macOS**, set **`DYLD_LIBRARY_PATH`** (or place `libsynx_c.dylib` next to your binary) so the loader finds the dylib.

**API:** `SynxEngine.parse`, `parseActive`, `stringify(json:)`, `format`, `parseTool`, `diff`, `compile`, `decompile`, `isSynxb` — throws `SynxEngineError` on FFI failure.

`Sources/CSynx/synx.h` mirrors `bindings/c-header/include/synx.h` (keep them in sync when the C API changes).

Details: [`bindings/swift/README.md`](../../bindings/swift/README.md).

---

## ☕ Kotlin / JVM (`bindings/kotlin`, JNA)

> v3.6.0 — **full parity** with **`synx-core`** via **`synx-c`** and [**JNA**](https://github.com/java-native-access/jna) (Gradle module **`synx-kotlin`**, not a pure-Kotlin parser).

**1. Build the native library** (repo root): `cargo build -p synx-c --release`.

**2. Set `SYNX_LIB_DIR`** to `target/release` (folder with `libsynx_c` / `synx_c.dll`), then from `bindings/kotlin`: `./gradlew test` (or `gradlew.bat test` on Windows). **JDK 17+** for Gradle; the build uses a **Foojay** toolchain hook in `settings.gradle.kts` when needed.

**API:** `SynxEngine.parse`, `parseActive`, `stringify`, `format`, `parseTool`, `diff`, `compile`, `decompile`, `isSynxb` — throws `SynxEngineError` on FFI failure. Consume **`publishToMavenLocal`** until Maven Central publication.

Details: [`bindings/kotlin/README.md`](../../bindings/kotlin/README.md).

---

## 🔮 Mojo (`bindings/mojo` + `synx_native`)

> v3.6.0 — **full parity** with **`synx-core`** by calling **`synx_native`** from Mojo ([**Python from Mojo**](https://docs.modular.com/mojo/manual/python/python-from-mojo)).

In this repository, **Python** is already **Rust** (`synx-core`) via **PyO3**, not a second parser. **Pure Mojo** reimplementation of the entire 3.6 grammar + `!active` engine + `.synxb` would be a **separate, large** project (like maintaining `synx-js` next to Rust). The supported path is the same as that Python model: **one engine**, Modular/CPython interop at the boundary.

**Install** the Python module Mojo will load:

```bash
pip install synx-format
```

(or `maturin develop` in `bindings/python` into the same environment.)

**API** (see `synx/interop.mojo`): `parse_json`, `parse_active_json`, `parse_tool_json`, `stringify_json`, `format_synx`, `diff_json`, `compile_hex`, `decompile_hex`, `is_synxb_hex`.
...
Details: [`bindings/mojo/README.md`](../../bindings/mojo/README.md).

---

## 🧩 C# / .NET Parser (`Synx.Core`)

> Added in v3.6.0.

The .NET implementation lives under `parsers/dotnet/` and provides:

**NuGet:** `dotnet add package APERTURESyndicate.Synx` — [nuget.org/packages/APERTURESyndicate.Synx](https://www.nuget.org/packages/APERTURESyndicate.Synx). The ID `Synx.Core` is already used on nuget.org by another package; this repo still uses the `Synx.Core` project name and `Synx` namespaces. Until the package appears on the feed, use a project reference or a local `.nupkg` (see [`parsers/dotnet/README.md`](../../parsers/dotnet/README.md)).

- `SynxFormat.Parse(text)` — static parsing
- `SynxFormat.ParseActive(text, options)` — `!active` markers/engine
- `SynxFormat.ParseTool(text)` — `!tool` tool-call parsing (reshape)
- `SynxFormat.ToJson(...)` — canonical JSON output

Quick example:

```csharp
using Synx;
var map = SynxFormat.ParseActive("!active\nport:env:default:8080 PORT\n");
...
Corpus replay tool: `parsers/dotnet/tools/Synx.FuzzReplay`.
...
Or (when published): `cargo install synx-cli`.
...
`synx watch` is also supported for live reload; see `synx --help`.
...
- **[SPECIFICATION (English)](../spec/SPECIFICATION_EN.md)**
- **[SPECIFICATION (Русский)](../spec/SPECIFICATION_RU.md)**
