...
**C++17** 头文件 `include/synx/synx.hpp` 封装原生库 **`synx-c`**（与 Rust **`synx-core`** 同一实现）：语法、`!active`、`!tool`、`.synxb`、规范 JSON、`diff` 均一致。

```bash
cargo build -p synx-c --release
```

包含路径：`bindings/cpp/include`、`bindings/c-header/include`。说明见 [`bindings/cpp/README.md`](../../bindings/cpp/README.md)。

## Go（`bindings/go`，cgo）— v3.6.0

通过 **cgo** 调用 **`synx-c`**，与 Rust 同一引擎。需 `CGO_ENABLED=1`、`cargo build -p synx-c --release`。**Windows** 需配置 `CGO_LDFLAGS` 与 `PATH` 加载 DLL — 见 [`bindings/go/README.md`](../../bindings/go/README.md)。

## Swift（`bindings/swift`，SwiftPM）— v3.6.0

SwiftPM 包 **`Synx`** 通过 **`CSynx`** 调用 **`synx-c`**，与 Rust 同一引擎（不是纯 Swift 语法重写）。先 `cargo build -p synx-c --release`，再用 `-Xlinker -L … -lsynx_c` 链接。macOS 可能需要 `DYLD_LIBRARY_PATH`。详见 [`bindings/swift/README.md`](../../bindings/swift/README.md)。

## Kotlin/JVM（`bindings/kotlin`，JNA）— v3.6.0

通过 **JNA** 加载 **`synx-c`** 的 **`SynxEngine`**，与 Rust 同一引擎。设置 `SYNX_LIB_DIR`，`./gradlew test`，**JDK 17+**。见 [`bindings/kotlin/README.md`](../../bindings/kotlin/README.md)。

## Mojo（`bindings/mojo`）— v3.6.0

**Mojo 经 CPython 调用 `synx_native`**（与本仓库 PyO3 一致），即 **`synx-core`**。`pip install synx-format` — [`bindings/mojo/README.md`](../../bindings/mojo/README.md)。
