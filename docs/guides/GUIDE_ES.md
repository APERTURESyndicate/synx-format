...
## SDK C++ (`bindings/cpp`) — v3.6.0

Capa **C++17** (`include/synx/synx.hpp`) sobre la biblioteca nativa **`synx-c`** (mismo motor que Rust `synx-core`): misma gramática, `!active`, `!tool`, `.synxb`, JSON canónico, `diff`.

```bash
cargo build -p synx-c --release
```

Includes: `bindings/cpp/include`, `bindings/c-header/include`. Ver [`bindings/cpp/README.md`](../../bindings/cpp/README.md).

## Go (`bindings/go`, cgo) — v3.6.0

Enlace **cgo** a **`synx-c`**, mismo motor que Rust. `CGO_ENABLED=1`, `cargo build -p synx-c --release`. **Windows:** `CGO_LDFLAGS` y `PATH` para la DLL — [`bindings/go/README.md`](../../bindings/go/README.md).

## Swift (`bindings/swift`, SwiftPM) — v3.6.0

Paquete SwiftPM **`Synx`** vía **C interop (`CSynx`)** con **`synx-c`** — el mismo motor que Rust, no un parser en Swift puro. Tras `cargo build -p synx-c --release`, enlazar con `-Xlinker -L … -lsynx_c`; en macOS puede hacer falta `DYLD_LIBRARY_PATH`. [`bindings/swift/README.md`](../../bindings/swift/README.md).

## Kotlin / JVM (`bindings/kotlin`, JNA) — v3.6.0

**`SynxEngine`** con **JNA** sobre **`synx-c`** — el mismo motor que Rust. `SYNX_LIB_DIR`, `./gradlew test`, **JDK 17+**. [`bindings/kotlin/README.md`](../../bindings/kotlin/README.md).

## Mojo (`bindings/mojo`) — v3.6.0

**Mojo → CPython `synx_native`**: mismo motor que Rust (PyO3), no un parser Mojo puro. `pip install synx-format`; [`bindings/mojo/README.md`](../../bindings/mojo/README.md).
